...
from torch_onnx_models.models import (
    CausalLMModel,
    Gemma3CausalLMModel,
    Gemma3ConditionalGenerationModel,
    Phi3CausalLMModel,
)
...
MODEL_MAP = {
    "gemma3": Gemma3ConditionalGenerationModel,
...
def create_text_gen_example_inputs(
    config: _configs.ArchitectureConfig, batch_size: int = 2, seq_len: int = 3, past_seq_len: int = 2
):
...
def convert_hf_model(
    ...
    Args:
        ...
        example_inputs_func: A function that creates example inputs for the model.
        model_id: The model ID on HuggingFace Hub.
        clear_metadata: Whether to clear debugging metadata from the ONNX model.
        run_cse: Whether to run common subexpression elimination on the ONNX model.
    ...
    example_inputs, dynamic_shapes, input_names, output_names = example_inputs_func(config)
    ...
    passes = ir.passes.PassManager(
        [
            ...
            *([common_passes.CommonSubexpressionEliminationPass()] if run_cse else []),
...
def convert_hf_text_gen_model(model_id: str, clear_metadata: bool = False) -> torch.onnx.ONNXProgram:
    """Convert a HuggingFace text generation model to ONNX.
    ...
    return convert_hf_model(
        model_class=model_class,
        config=architecture_config,
        example_inputs_func=create_text_gen_example_inputs,
        model_id=model_id,
